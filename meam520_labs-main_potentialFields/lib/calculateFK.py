...
class FK():
    def __init__(self):
        ...
        self.alpha7 =  0.0
        # Joint limits
        self.lowerLim = np.array([-2.8973, -1.7628, -2.8973, -3.0718, -2.8973, -0.0175, -2.8973]).reshape((1, 7))    # Lower joint limits in radians ** This doesnot include gripper
        self.upperLim = np.array([ 2.8973,  1.7628,  2.8973, -0.0698,  2.8973,  3.7525,  2.8973]).reshape((1, 7))          # Upper joint limits in radians (grip inmm)
...
joint of the robot or end effector
                  Each row contains the [x,y,z] coordinates in the world frame of 
the respective joint's center in meters.
        ...
        x[6, :] = np.transpose( Ai[0] @ Ai[1] @ Ai[2] @ Ai[3] @ Ai[4] @ Ai[5] @ 
joint7_pos )
...
of rotation for each joint in the
                                 world frame
        """
        return ()
    def compute_Ai(self, q):
        ...
        q - 1x7 vector of joint angles [q0, q1, q2, q3, q4, q5, q6]
        OUTPUTS:
        Ai: - 4x4 list of np array of homogenous transformations describing the FK 
of the robot. Transformations are not
